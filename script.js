...
canvas.addEventListener('pointermove', (e)=>{
  ...
  } else if(drawing && previewRect){
    restoreCanvasFromHistoryImage(history[historyIndex]?.image || null, false);
...
textInput.type='text'; document.body.appendChild(textInput);
textInput.style.position='absolute'; textInput.style.display='none';
function openTextInputAt(x,y){
  ...
  function commit(){ 
    ...
    if(txt){ texts.push({x,y,text:txt,color,size:20}); pushHistory(); }
...
function restoreCanvasFromHistoryImage(dataURL, drawTexts=true){
  if(!dataURL){ ctx.clearRect(0,0,canvas.width,canvas.height); if(bgImage)ctx.drawImage(bgImage,0,0); if(drawTexts) renderTexts(); return; }
  const img=new Image();
  img.onload=()=>{
    ctx.clearRect(0,0,canvas.width,canvas.height);
    ctx.drawImage(img,0,0);
    if(drawTexts) renderTexts();
  };
  img.src=dataURL;
}
